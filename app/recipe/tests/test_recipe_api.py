import tempfile  # allows to generate temporary files
import os  # creates path, checks if files exists on the system

from PIL import Image  # imports image class, creates test images
...
class RecipeImageUploadTests(TestCase):
    ...
    def test_upload_image_to_recipe(self):
        ...
        with tempfile.NamedTemporaryFile(suffix='.jpg') as ntf:
            img = Image.new('RGB', (10, 10))  # black square 10 pix by 10 pix
            img.save(ntf, format='JPEG')
            ntf.seek(0)  # set the pointer to the beginning of the file
            res = self.client.post(url, {'image': ntf}, format='multipart')
            # form that consist data
